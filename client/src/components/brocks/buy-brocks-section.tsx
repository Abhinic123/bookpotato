...
interface BuyBrocksSectionProps {
  compactHeader?: boolean;
  initialPackageId?: string;
  showTitle?: boolean;
}

export default function BuyBrocksSection({ 
  compactHeader = false, 
  initialPackageId = "",
  showTitle = true 
}: BuyBrocksSectionProps) {
  ...
  // Update package selection when initialPackageId changes
  useEffect(() => {
    if (initialPackageId) {
      setSelectedPackage(initialPackageId);
      form.setValue("package", initialPackageId);
    }
  ...
  return (
    <div className="space-y-6">
      {/* Title (only show if not in compact mode or explicitly requested) */}
      ...
      {/* Current Balance - Compact or Full */}
      {compactHeader ? (
      ...
      <div className="space-y-4">
        ...
        {packagesLoading ? (
          ...
          <div className={`grid gap-4 ${compactHeader ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'}`}>
